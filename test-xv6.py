...
import argparse, os, inspect, re, signal, subprocess, sys, time, fcntl
...
class QEMU(object):
    ...
    def read(self):
        # Handle non-blocking read properly
        try:
    ...
    def monitor(self, *regexps, progress="", timeout):
        # Read more frequently (10x per second instead of 1x per second)
...
def test_usertests(test=""):
    ...
    if args.q:
        ...
        opt += " " + test
    q = QEMU(True)
    time.sleep(3)  # Wait longer for QEMU to boot
    q.cmd("usertests" + opt + "\n")
